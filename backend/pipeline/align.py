...
def stretch_to_duration(
    ...
    if duration_error <= 0.06:
        ...
        try:
            safe_stretch_rate = max(0.65, min(stretch_rate, 1.45)) # Cap phase-vocoder tearing
            if _ffmpeg_time_stretch(audio_path, temp_output, safe_stretch_rate, sr):
                stretched, _ = librosa.load(temp_output, sr=sample_rate, mono=True)
            else:
                stretched = librosa.effects.time_stretch(y, rate=safe_stretch_rate)
        finally:
...
def align_prosody(
    ...
    try:
        ...
        # Enhance pause alignment by heavily weighting energy against silence drift
        rms_src = librosa.feature.rms(y=y_src, hop_length=hop_length)
        rms_dub = librosa.feature.rms(y=y_dub, hop_length=hop_length)
        rms_src = rms_src / (np.max(rms_src) + 1e-6)
        rms_dub = rms_dub / (np.max(rms_dub) + 1e-6)
        
        # Combine MFCC and weighted RMS (30.0 forces voice to align with voice)
        features_src = np.vstack([mfcc_src, 30.0 * rms_src])
        features_dub = np.vstack([mfcc_dub, 30.0 * rms_dub])

        D, wp = librosa.sequence.dtw(X=features_src, Y=features_dub, metric="euclidean")
        wp = wp[::-1]
        ...
        # --- SYLLABLE-PRESERVING ALIGNMENT CONSTRAINT ---
        # Detect where the English AI voice is actually speaking vs pausing
        non_silent_intervals = librosa.effects.split(y_dub, top_db=32, frame_length=512, hop_length=hop_length)
        
        # Smooth the warped timeline so words don't get chopped internally
        # We enforce a constant stretch rate INSIDE each spoken word!
        for interval in non_silent_intervals:
            start_dub_sample, end_dub_sample = interval
            start_dub_time = start_dub_sample / sr
            end_dub_time = end_dub_sample / sr
            
            # Find which source timeline indexes map to this word
            idx_start = np.searchsorted(dubbed_times, start_dub_time)
            idx_end = np.searchsorted(dubbed_times, end_dub_time)
            
            if idx_start < idx_end and idx_end < len(source_times):
                # The word occurs in the source between these source times
                word_source_start = source_times[idx_start]
                word_source_end = source_times[idx_end]
                
                # Make the stretch strictly uniform inside this word
                source_times[idx_start:idx_end] = np.linspace(
                    word_source_start, word_source_end, idx_end - idx_start
                )
                dubbed_times[idx_start:idx_end] = np.linspace(
                    start_dub_time, end_dub_time, idx_end - idx_start
                )
        ...
        # Post-DTW Soft Gate to suppress artifacts generated by squishing/expanding background noise
        try:
            from scipy.ndimage import gaussian_filter1d
            alg_rms = librosa.feature.rms(y=aligned_audio, frame_length=512, hop_length=256)[0]
            alg_gain = np.interp(alg_rms, [0.005, 0.025], [0.0, 1.0])
            alg_gain = gaussian_filter1d(alg_gain, sigma=4.0)
            n_frames = len(alg_gain)
            frame_times = librosa.frames_to_time(np.arange(n_frames), sr=sr, hop_length=256)
            sample_times = np.arange(len(aligned_audio)) / sr
            sample_gate = np.interp(sample_times, frame_times, alg_gain)
            aligned_audio = aligned_audio * sample_gate.astype(np.float32)
        except Exception as e:
            print(f"         [WARNING] Post-DTW noise gating failed: {e}")
